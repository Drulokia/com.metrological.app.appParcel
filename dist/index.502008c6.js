 ...
 */ var APP_com_metrological_app_parcelapp = function() {
   ...
   */ const settings = {};
    ...
    const dotGrab = function() {
        ...
        for(let i = 0; i < keys.length; i++)obj = obj[keys[i]] = obj[keys[i]] !== undefined ? obj[keys[i]] : {};
    ...
    var Settings$1 = {
        get (type, key) {
            ...
            const val1 = dotGrab(settings[type], key);
            return val1 !== undefined ? val1 : fallback;
        },
        has (type, key) {
   ...
   */ class ColorShift extends Lightning$1.shaders.WebGLDefaultShader {
        set brightness(v1) {
            this._brightness = (v1 - 50) / 100;
            this.redraw();
        }
        set contrast(v1) {
            this._contrast = (v1 + 50) / 100;
            this.redraw();
        }
        set gamma(v1) {
            this._gamma = (v1 + 50) / 100;
            this.redraw();
        }
        setupUniforms(operation) {
            super.setupUniforms(operation);
            const gl = this.gl;
            this._setUniform("colorAdjust", [
   ...
   */ const formatLocale = (locale)=>{
        if (locale && locale.length === 2) return "".concat(locale.toLowerCase(), "-").concat(locale.toUpperCase());
        else return locale;
    };
    ...
    const hasOrAskForGeoLocationPermission = ()=>{
        return new Promise((resolve)=>{
            ...
            if ("permissions" in navigator && typeof navigator.permissions.query === "function") navigator.permissions.query({
                name: "geolocation"
            }).then((status)=>{
    ...
    const getLatLon = (defaultValue)=>{
        return new Promise((resolve)=>{
            hasOrAskForGeoLocationPermission().then((granted)=>{
                if (granted === true) {
                    if ("geolocation" in navigator) navigator.geolocation.getCurrentPosition((result)=>result && result.coords && resolve([
                    ...
                    else return queryForLatLon().then((result)=>resolve(result || defaultValue));
                } else return queryForLatLon().then((result)=>resolve(result || defaultValue));
            });
        });
    };
    const queryForLatLon = ()=>{
        return new Promise((resolve)=>{
            fetch("https://geolocation-db.com/json/").then((response)=>response.json()).then((_ref)=>{
                let { latitude , longitude  } = _ref;
                return latitude && longitude ? resolve([
                    latitude,
                    longitude
                ]) : resolve(false);
            }).catch(()=>resolve(false));
   ...
   */ const defaultChannels = [
        {
            ...
            program: {
                ...
                startTime: new Date(new Date() - 300000).toUTCString(),
        ...
        },
        {
            ...
            program: {
                ...
                startTime: new Date(new Date() - 1200000).toUTCString(),
        ...
        },
        {
            ...
            program: {
                ...
                startTime: new Date(new Date() - 600000).toUTCString(),
            ...
            return new Promise((resolve, reject)=>{
                if (number) {
                    ...
                    if (newChannel) {
                        ...
                        resolve(channel);
                    } else reject("Channel not found");
                } else reject("No channel number supplied");
            });
        }
    };
    const initTV = (config)=>{
        methods = {};
        if (config.getChannel && typeof config.getChannel === "function") methods.getChannel = config.getChannel;
        if (config.getProgram && typeof config.getProgram === "function") methods.getProgram = config.getProgram;
        if (config.setChannel && typeof config.setChannel === "function") methods.setChannel = config.setChannel;
        if (config.emit && typeof config.emit === "function") config.emit(emit$1);
    }; // public API
    /*
   * If not stated otherwise in this file or this component's LICENSE file the
   * following copyright and licenses apply:
   ...
   */ const initPin = (config)=>{
        config.submit && typeof config.submit;
        config.check && typeof config.check;
    };
    var executeAsPromise = function(method) {
        ...
        let result;
        if (method && typeof method === "function") try {
        ...
        if (result !== null && typeof result === "object" && result.then && typeof result.then === "function") return result;
   ...
   */ var autoSetupMixin = function(sourceObject) {
        ...
        return Object.keys(sourceObject).reduce((obj, key)=>{
            if (typeof sourceObject[key] === "function") obj[key] = function() {
                doSetup();
                return sourceObject[key].apply(sourceObject, arguments);
            };
            else if (typeof Object.getOwnPropertyDescriptor(sourceObject, key).get === "function") obj.__defineGetter__(key, function() {
                doSetup();
                return Object.getOwnPropertyDescriptor(sourceObject, key).get.apply(sourceObject);
            });
            else if (typeof Object.getOwnPropertyDescriptor(sourceObject, key).set === "function") obj.__defineSetter__(key, function() {
                doSetup();
                return Object.getOwnPropertyDescriptor(sourceObject, key).set.sourceObject[key].apply(sourceObject, arguments);
            });
            else obj[key] = sourceObject[key];
            return obj;
        }, {});
    };
    /*
   ...
   */ var VideoTexture = ()=>{
        return class VideoTexture extends Lightning.Component {
            ...
            start() {
                const stage = this.stage;
                this._lastTime = 0;
                if (!this._updateVideoTexture) this._updateVideoTexture = ()=>{
   ...
   */ let mediaUrl$1 = (url)=>url;
    ...
    const initVideoPlayer = (config)=>{
        if (config.mediaUrl) mediaUrl$1 = config.mediaUrl;
    ...
    const withPrecision = (val1)=>Math.round(precision * val1) + "px";
    ...
    const setupVideoTag = ()=>{
        ...
        if (videoEls && videoEls.length) return videoEls[0];
        else {
    ...
    const registerEventListeners = ()=>{
        ...
        Object.keys(events$1).forEach((event)=>{
            const handler = (e)=>{
                ...
                if (metrics && metrics[event] && typeof metrics[event] === "function") metrics[event]({
    ...
    const videoPlayerPlugin = {
        ...
        position () {
            ...
            if (textureMode === true) videoTexture.position(top, left);
        },
        size () {
            ...
            if (textureMode === true) videoTexture.size(width, height);
        },
        area () {
            let top = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
        ...
        open (url) {
            ...
            this.hide();
            deregisterEventListeners();
            if (this.src == url) this.clear().then(this.open(url, config));
            else {
                const adConfig = {
                    enabled: state$1.adsEnabled,
                    duration: 300
                };
                if (config.videoId) adConfig.caid = config.videoId;
        ...
        seek (time) {
            ...
            if (state$1.playAfterSeek === null) state$1.playAfterSeek = !!state$1.playing;
        ...
        show () {
            if (!this.canInteract) return;
            if (textureMode === true) videoTexture.show();
            else {
        ...
        hide () {
            if (!this.canInteract) return;
            if (textureMode === true) videoTexture.hide();
            else {
                videoEl.style.display = "none";
                videoEl.style.visibility = "hidden";
            }
        ...
        get visible () {
            if (textureMode === true) return videoTexture.isVisible;
            else return videoEl && videoEl.style.display === "block";
        },
        get adsEnabled () {
   ...
   */ let consumer;
    ...
    const initAds = (config)=>{
        if (config.getAds) getAds = config.getAds;
    };
    const state = {
        active: false
    };
        ...
        if (callbacks && callbacks[event]) {
            ...
            return callbacks[event].reduce((promise, url)=>{
                return promise.then(()=>fetch(url) // always resolve, also in case of a fetch error (so we don't block firing the rest of the beacons for this event)
                    ...
                    .then((response)=>{
                        if (response.status === 200) fireOnConsumer("Beacon" + event + "Sent");
                        else fireOnConsumer("Beacon" + event + "Failed" + response.status);
                        Promise.resolve(null);
                    }).catch(()=>{
                        Promise.resolve(null);
                    }));
            }, Promise.resolve(null));
        } else Log$1.info("Ad", "No callback found for " + event);
    };
   ...
   */ let warned = false;
    const deprecated = function() {
        ...
        if (force === true || warned === false) console.warn([
     ...
     */ loadFromObject(trObj) {
            ...
            this.__trObj = trObj;
            for (const lang of Object.values(this.__trObj))for (const str of Object.keys(lang))lang[str] = new LocalizedString(lang[str]);
        }
    }
    /**
   ...
   */ let basePath;
    ...
    const initUtils = (config)=>{
        ...
        if (config.proxyUrl) proxyUrl = ensureUrlWithProtocol(config.proxyUrl);
    };
    var Utils = {
        asset (relPath) {
    ...
    const ensureUrlWithProtocol = (url)=>{
        if (/^\/[^/]/i.test(url) && /^(?:file:)/i.test(window.location.protocol)) return window.location.protocol + "//" + url;
   ...
   */ let meta = {};
    ...
    const setLanguage = (lng)=>{
        language = null;
        return new Promise((resolve, reject)=>{
            if (lng in translations) language = lng;
            else {
                if ("map" in meta && lng in meta.map && meta.map[lng] in translations) language = meta.map[lng];
                else if ("default" in meta && meta.default in translations) {
                    const error = "Translations for Language " + language + " not found. Using default language " + meta.default;
                    Log$1.warn(error);
                    language = meta.default;
                } else {
                    const error1 = "Translations for Language " + language + " not found.";
                    Log$1.error(error1);
                    reject(error1);
                }
            }
            if (language) {
                ...
                if (typeof translationsObj === "object") resolve();
   ...
   */ const registry = {
    ...
    var Registry = {
        ...
        setTimeout (cb, timeout) {
            for(var _len = arguments.length, params = new Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++)params[_key - 2] = arguments[_key];
        ...
        clearTimeout (timeoutId) {
            if (registry.timeouts.indexOf(timeoutId) > -1) {
                ...
                clearTimeout(timeoutId);
            } else Log$1.error("Clear Timeout", "ID " + timeoutId + " not found");
        },
        clearTimeouts () {
        ...
        clearInterval (intervalId) {
            if (registry.intervals.indexOf(intervalId) > -1) {
                ...
                clearInterval(intervalId);
            } else Log$1.error("Clear Interval", "ID " + intervalId + " not found");
        },
        clearIntervals () {
        ...
        removeEventListener (target, event, handler) {
            ...
            } else Log$1.error("Remove eventListener", "Not found", "Target", target, "Event: " + event, "Handler", handler.toString());
        },
        ...
        removeEventListeners (target, event) {
            if (target && event) {
                const targetIndex = registry.targets.indexOf(target);
                if (targetIndex > -1) registry.eventListeners[targetIndex][event].forEach((handler)=>{
                    this.removeEventListener(target, event, handler);
                });
            } else if (target) {
                const targetIndex1 = registry.targets.indexOf(target);
                if (targetIndex1 > -1) Object.keys(registry.eventListeners[targetIndex1]).forEach((_event)=>{
                    this.removeEventListeners(target, _event);
                });
            } else Object.keys(registry.eventListeners).forEach((targetIndex)=>{
   ...
   * limitations under the License.
   */ const isObject$1 = (v1)=>{
        return typeof v1 === "object" && v1 !== null;
    };
    const isString$1 = (v1)=>{
   ...
   */ let colors = {
    ...
    const cleanUpNormalizedColors = (color)=>{
        for(let c in normalizedColors)if (c.indexOf(color) > -1) delete normalizedColors[c];
    };
   ...
   */ let AppInstance;
    ...
    const fontLoader = (fonts, store)=>new Promise((resolve, reject)=>{
            fonts.map((_ref)=>{
                let { family , url , urls , descriptors  } = _ref;
                return ()=>{
    ...
    function Application(App, appData, platformSettings) {
        ...
        return class Application extends Lightning$1.Application {
            constructor(options){
                ...
                if (options.stage.canvas) config.stage.canvas = options.stage.canvas;
                super(config);
                this.config = config;
            }
            static _template() {
            ...
            _setup() {
                Promise.all([
                    ...
                    if (platformSettings.showVersion) this.childList.a({
                        ref: "VersionLabel",
                        type: VersionLabel,
                    ...
                    if (platformSettings.showFps) this.childList.a({
                        ref: "FpsCounter",
                        type: FpsIndicator,
                        zIndex: 1
                    });
            ...
            closeApp() {
                ...
                if (platformSettings.onClose && typeof platformSettings.onClose === "function") platformSettings.onClose(...arguments);
                else this.close();
            }
            close() {
            ...
            cleanupFonts() {
                if ("delete" in document.fonts) customFontFaces.forEach((fontFace)=>{
                    Log$1.info("Removing font", fontFace.family);
                    document.fonts.delete(fontFace);
                });
                else Log$1.info("No support for removing manually-added fonts");
            }
            loadLanguage(config) {
            ...
            loadColors(config) {
                ...
                if (config && (typeof config === "string" || typeof config === "object")) file = config;
   ...
   */ class ScaledImageTexture extends Lightning$1.textures.ImageTexture {
        ...
        getNonDefaults() {
            ...
            if (this._src) obj.src = this._src;
            return obj;
        }
    }
    /*
   ...
   */ const events = [
    ...
    const initMediaPlayer = (config)=>{
        if (config.mediaUrl) mediaUrl = config.mediaUrl;
    };
    class Mediaplayer extends Lightning$1.Component {
        ...
        set skipRenderToTexture(v1) {
            this._skipRenderToTexture = v1;
        }
        get textureMode() {
        ...
        _init() {
            ...
            if (this.textureMode && !this._skipRenderToTexture) this._createVideoTexture();
            this.eventHandlers = [];
        }
        _registerListeners() {
            events.forEach((event)=>{
                const handler = (e)=>{
                    if (this._metrics && this._metrics[event] && typeof this._metrics[event] === "function") this._metrics[event]({
        ...
        updateSettings() {
            ...
            if (this._consumer && this._consumer.getMediaplayerSettings) // Allow consumer to add settings.
            settings = Object.assign(settings, this._consumer.getMediaplayerSettings());
            if (!Lightning$1.Utils.equalValues(this._stream, settings.stream)) {
                if (settings.stream && settings.stream.keySystem) navigator.requestMediaKeySystemAccess(settings.stream.keySystem.id, settings.stream.keySystem.config).then((keySystemAccess)=>{
                    ...
                    if (Settings$1.get("app", "hls")) {
                        if (!window.Hls) window.Hls = class Hls {
                            static isSupported() {
                    ...
                    } else this.open(settings.stream.src);
                } else this.close();
        ...
        _setHide(hide) {
            if (this.textureMode) this.tag("Video").setSmooth("alpha", hide ? 0 : 1);
            else this.videoEl.style.visibility = hide ? "hidden" : "visible";
        }
        open(url) {
            ...
            if (this.videoEl.getAttribute("src") === url) this.close();
            ...
            this._setHide(settings.hide);
            this._setVideoArea(settings.videoPosition || [
                0,
                0,
                1920,
                1080
        ...
        playPause() {
            if (this.isPlaying()) this.doPause();
            else this.doPlay();
        }
        get muted() {
        ...
        set muted(v1) {
            this.videoEl.muted = v1;
        }
        get loop() {
            return this.videoEl.loop;
        }
        set loop(v1) {
            this.videoEl.loop = v1;
        }
        isPlaying() {
        ...
        seek(time) {
            ...
            if (absolute) this.videoEl.currentTime = time;
        ...
        _setVideoArea(videoPos) {
            if (Lightning$1.Utils.equalValues(this._videoPos, videoPos)) return;
            this._videoPos = videoPos;
            if (this.textureMode) this.videoTextureView.patch({
                smooth: {
                    x: videoPos[0],
                    y: videoPos[1],
        ...
        _fireConsumer(event, args) {
            if (this._consumer) this._consumer.fire(event, args);
        }
   ...
   */ const stripRegex = function(route) {
        ...
        if (hasRegex.test(route)) route = route.replace(hasRegex, char);
        return route;
    };
    /*
   * If not stated otherwise in this file or this component's LICENSE file the
   ...
   */ const createRegister = (flags)=>{
        ...
        [
            ...Object.keys(flags),
            ...Object.getOwnPropertySymbols(flags)
        ].forEach((key)=>{
   ...
   */ class Request {
       ...
       */ this._copiedHistoryState = null; // if there are arguments attached to navigate()
            ...
            if (isObject(navArgs)) this._register = createRegister(navArgs);
            else if (isBoolean(navArgs)) // if second navigate() argument is explicitly
        ...
        set copiedHistoryState(v1) {
            this._copiedHistoryState = v1;
        }
   ...
   */ const getRoutesByFloor = (floor)=>{
        ...
        for (let [route] of routes.entries())if (getFloor(route) === floor) matches.push(route);
        return matches;
    };
    /**
   ...
   */ const getRouteByHash = (hash)=>{
        ...
        let matches = candidates.filter((route)=>{
            ...
            if (hasRegex.test(route)) {
                ...
                if (regMatches && regMatches.length) route = regMatches.reduce((fullRoute, regex)=>{
            ...
            for(let i = 0, j = routeParts.length; i < j; i++){
                ...
                if (hasLookupId.test(routePart)) {
                    ...
                    const regMatches1 = /\/([^\/]+)\/([igm]{0,3})/.exec(routeRegex);
                    if (regMatches1 && regMatches1.length) {
                        const expression = regMatches1[1];
                        const modifiers = regMatches1[2];
                        const regex = new RegExp("^/".concat(expression, "$"), modifiers);
                        if (!regex.test(hashPart)) isMatching = false;
                    }
                } else if (isNamedGroup.test(routePart)) continue;
                else if (hashPart && routePart.toLowerCase() !== hashPart.toLowerCase()) isMatching = false;
            }
            return isMatching;
        });
        if (matches.length) {
            if (matches.indexOf(hash) !== -1) {
                ...
                // but still we test
                if (routeExists(matches[0])) return routes.get(matches[0]);
            }
        }
        return false;
    };
    const getValuesFromHash = function() {
        ...
        return routeParts.reduce((storage, value, index)=>{
            ...
            if (match && match.length) storage.set(match[1], decodeURIComponent(hashParts[index].replace(/^\//, "")));
            return storage;
        }, new Map());
    };
    const getOption = (stack, prop)=>{
        // eslint-disable-next-line
        if (stack && stack.hasOwnProperty(prop)) return stack[prop];
   ...
   */ const createRequest = (url, args, store)=>{
    ...
    const getHashByName = (obj)=>{
        if (!obj.to && !obj.name) return false;
        ...
        if (hasDynamicGroup.test(route)) {
            ...
            if (obj.query) return "".concat(hash).concat(objectToQueryString(obj.query));
        }
        return hash;
    };
    const getRouteByName = (name)=>{
        for (let [path, route] of routes.entries()){
            if (route.name === name) return path;
        }
        return false;
    };
    const keepActivePageAlive = (route, request)=>{
        if (isString(route)) {
            ...
            if (routes.has(route)) route = routes.get(route);
            else return false;
        }
        const register = request.register;
        const routeOptions = route.options;
        if (register.has("keepAlive")) return register.get("keepAlive");
        else if (routeOptions && routeOptions.keepAlive) return routeOptions.keepAlive;
        return false;
    };
    /*
   * If not stated otherwise in this file or this component's LICENSE file the
   ...
   */ var emit = function(page) {
        ...
        events.forEach((e)=>{
            ...
            if (isFunction(page[event])) page[event](params);
        });
    };
    /*
   * If not stated otherwise in this file or this component's LICENSE file the
   ...
   */ const updateWidgets = (widgets, page)=>{
        ...
        widgetsHost.forEach((widget)=>{
            ...
            if (widget.visible) emit(widget, [
                "activated"
            ], page);
        });
        if (app.state === "Widgets" && activeWidget && !activeWidget.visible) app._setState("");
    };
   ...
   */ const focusWidget = (name)=>{
        ...
        if (widget) {
            ...
            if (app.state === "Widgets") app.reload(activeWidget);
            else app._setState("Widgets", [
                activeWidget
            ]);
        }
    };
   ...
   */ let history = [];
    const updateHistory = (request)=>{
        ...
        if (store) {
            ...
            if (location === -1 || routerConfig.get("storeSameHash")) history.push({
                hash: toStore,
                state: stateObject
            });
            else {
    ...
    const locationInHistory = (hash)=>{
        for(let i = 0; i < history.length; i++){
            if (history[i].hash === hash) return i;
        }
        return -1;
    };
    const getHistoryState = (hash)=>{
        ...
        if (history.length) {
            ...
            if (!hash) {
                ...
                state = record.state;
            } else if (locationInHistory(hash) !== -1) {
                const record1 = history[locationInHistory(hash)];
                state = record1.state;
            }
        }
        return state;
    ...
    const replaceHistoryState = function() {
        ...
        if (location !== -1 && isObject(state)) history[location].state = state;
    };
    const getStateObject = (page, request)=>{
        ...
        if (request.isSharedInstance) {
            if (request.copiedHistoryState) return request.copiedHistoryState;
        } else if (page && isFunction(page.historyState)) return page.historyState();
        return null;
    };
    const getHistory = ()=>{
   ...
   */ let previousState;
    const mixin = (app)=>{
        ...
        if (app.pages) pagesHost = app.pages.childList;
    ...
    const bootRouter = (config, instance)=>{
        let { appInstance , routes  } = config; // if instance is provided and it's and Lightning Component instance
        if (instance && isPage(instance)) app = instance;
        if (!app) app = appInstance || AppInstance;
        application = app.application;
        pagesHost = application.childList;
        stage = app.stage;
        routerConfig = getConfigMap();
        mixin(app);
        if (isArray(routes)) setup(config);
    ...
    const setup = (config)=>{
        ...
        config.routes.forEach((r)=>{
            ...
            } else console.error("".concat(path, " already exists in routes configuration"));
        });
    };
    const init = (config)=>{
        rootHash = config.root;
        if (isFunction(config.boot)) bootRequest = config.boot;
        if (isBoolean(config.updateHash)) updateHash = config.updateHash;
        if (isFunction(config.beforeEachRoute)) beforeEachRoute = config.beforeEachRoute;
        if (isFunction(config.afterEachRoute)) afterEachRoute = config.afterEachRoute;
        if (config.bootComponent) {
            ...
            if (isPage(config.bootComponent)) config.routes.push({
                path: "$",
            ...
            else console.error("[Router]: ".concat(config.bootComponent, " is not a valid boot component"));
        }
        initialised = true;
    };
    const storeComponent = (route, type)=>{
        if (components.has(route)) components.set(route, type);
    };
    const getComponent = (route)=>{
        if (components.has(route)) return components.get(route);
        return null;
    };
    /**
   ...
   */ const mustUpdateLocationHash = ()=>{
        if (!routerConfig || !routerConfig.size) return false;
   ...
   */ const onRequestResolved = (request)=>{
        ...
        if (getOption(route.options, "clearHistory")) setHistory([]);
        else if (hash && !isWildcard.test(route.path)) updateHistory(request);
         // we only update the stackLocation if a route
        // is not expired before it resolves
        storeComponent(route.path, page);
        if (request.isSharedInstance || !request.isCreated) emit(page, "changed");
        else if (request.isCreated) emit(page, "mounted");
         // only update widgets if we have a host
        if (widgetsHost) updateWidgets(route.widgets, page);
         // we want to clean up if there is an
        // active page that is not being shared
        // between current and previous route
        if (getActivePage() && !request.isSharedInstance) cleanUp(activePage, request);
         // provide history object to active page
        if (register.get(symbols.historyState) && isFunction(page.historyState)) page.historyState(register.get(symbols.historyState));
        setActivePage(page);
        activeHash = request.hash;
        activeRoute = route.path; // cleanup all cancelled requests
        for (let request1 of navigateQueue.values())if (request1.isCancelled && request1.hash) navigateQueue.delete(request1.hash);
    ...
    const cleanUp = (page, request)=>{
        ...
        if (isFromHistory && (destroyOnBack || lazyDestroy)) doCleanup = true;
         // clean up if lazyDestroy is enabled and the keepAlive flag
        // in navigation register is false
        if (lazyDestroy && !keepAlive) doCleanup = true;
         // if the current and new request share the same route blueprint
        if (activeRoute === request.route.path) doCleanup = true;
        if (doCleanup) {
            ...
            if (routerConfig.get("gcOnUnload")) stage.gc();
        } else // If we're not removing the page we need to
   ...
   */ const isFunction = (v1)=>{
    ...
    const isBoolean = (v1)=>{
        return typeof v1 === "boolean";
    };
    const isPage = (v1)=>{
        if (v1 instanceof Lightning$1.Element || isComponentConstructor(v1)) return true;
        return false;
    };
    ...
    const ucfirst = (v1)=>{
        return "".concat(v1.charAt(0).toUpperCase()).concat(v1.slice(1));
    };
    const isString = (v1)=>{
        return typeof v1 === "string";
    };
    ...
    const getQueryStringParams = function() {
        ...
        if ((hash === "$" || !hash) && resumeHash) {
            if (isString(resumeHash)) hash = resumeHash;
        }
        let parse = "";
        const getQuery = /([?&].*)/;
        const matches = getQuery.exec(hash);
        const params = {};
        if (document.location && document.location.search) parse = document.location.search;
   ...
   */ const dataHooks = {
    ...
    const execProvider = (request, emitProvided)=>{
        ...
        return provider.request(request.page, {
            ...
            if (emitProvided) emit(request.page, "dataProvided");
        }).catch((e)=>{
    ...
    const addPersistData = (_ref)=>{
        let { page , route , hash , register =new Map()  } = _ref;
        ...
        for (let [name, value] of pageData)params[name] = value;
        if (queryParams) params[symbols.queryParams] = queryParams;
         // check navigation register for persistent data
        if (register.size) {
            const obj = {};
            for (let [k, v1] of register)obj[k] = v1;
            page.persist = obj;
   ...
   */ const isPageExpired = (page)=>{
        if (!page[symbols.expires]) return false;
   ...
   */ const executeTransition = function(pageIn) {
        ...
        if (pageIn.easing) console.warn("easing() method is deprecated and will be removed. Use pageTransition()");
         // default behaviour is a visibility toggle
        if (!hasCustomTransitions || transitionsDisabled) {
            pageIn.visible = true;
            if (pageOut) pageOut.visible = false;
            return Promise.resolve();
        }
        if (transition) {
            let type;
            ...
            if (isPromise(type)) return type;
            if (isString(type)) {
                const fn = Transitions[type];
                if (fn) return fn(pageIn, pageOut);
            } // keep backwards compatible for now
            if (pageIn.smoothIn) {
                // provide a smooth function that resolves itself
                // on transition finish
                const smooth = function(p, v1) {
                    let args = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
                    return new Promise((resolve)=>{
                        pageIn.visible = true;
                        pageIn.setSmooth(p, v1, args);
   ...
   * */ const load = async (request)=>{
        ...
        try {
            ...
            if (request && !request.isCancelled) {
                ...
                if (!request.isSharedInstance && !request.isCancelled) await executeTransition(request.page, getActivePage());
            } else expired = true;
             // on expired we only cleanup
            if (expired || request.isCancelled) {
                Log$1.debug("[router]:", "Rejected ".concat(request.hash, " because route to ").concat(getLastHash(), " started"));
                if (request.isCreated && !request.isSharedInstance) // remove from render-tree
            ...
            if (!request1.route) console.error(request1);
            else if (!expired) {
                // @todo: revisit
                const { route  } = request1; // clean up history if modifier is set
                if (getOption(route.options, "clearHistory")) setHistory([]);
                else if (!isWildcard.test(route.path)) updateHistory(request1);
                if (request1.isCreated && !request1.isSharedInstance) // remove from render-tree
                pagesHost.remove(request1.page);
                handleError(request1);
            }
        }
    };
    const loader = async (request)=>{
        ...
        if (!isConstruct) {
            ...
            if (hasProvider(route.path)) {
                if (isPageExpired(type) || type[symbols.hash] !== hash) provide = true;
            }
            ...
            if (route.path === currentRoute) {
                ...
                // being overridden.
                if (isFunction(request.page.historyState)) request.copiedHistoryState = request.page.historyState();
            }
        } else {
            request.page = createComponent(stage, type);
            pagesHost.a(request.page); // test if need to request data provider
            if (hasProvider(route.path)) provide = true;
        ...
        try {
            if (provide) {
                ...
                // we're processing
                const { type: loadType , provider  } = getProvider(route); // update running request
                request.provider = provider;
                request.providerType = loadType;
                await dataHooks[loadType](request); // we early exit if the current request is expired
                if (hash !== getLastHash()) return false;
                else {
                    if (request.providerType !== "after") emit(request.page, "dataProvided");
                     // resolve promise
                    return request;
                }
            } else {
    ...
    const handleError = (request)=>{
        if (request && request.error) console.error(request.error);
        else if (request) Log$1.error(request);
        if (request.page && routeExists("!")) navigate("!", {
            request
        }, false);
    };
   ...
   */ class RoutedApp extends Lightning$1.Component {
        ...
        static _states() {
           ...
           */ if (!isBoolean(restoreFocus) || restoreFocus === true) Router.focusPage();
                    }
                }
            ];
        }
   ...
   */ const startRouter = (config, instance)=>{
    ...
    const start = ()=>{
        ...
        if (isWildcard.test(hash) && hash !== bootKey) hash = "";
         // store resume point for manual resume
        resumeHash = isDirectLoad ? rootHash : hash || rootHash;
        const ready = ()=>{
            if (!hash && rootHash) {
                if (isString(rootHash)) navigate(rootHash);
                else if (isFunction(rootHash)) rootHash().then((res)=>{
                    if (isObject(res)) navigate(res.path, res.params);
                    else navigate(res);
                });
            } else {
                queue(hash);
        ...
        } else if (isFunction(bootRequest)) bootRequest(params).then(()=>{
            ready();
        }).catch((e)=>{
            handleBootError(e);
        });
   ...
   */ const navigate = function(url) {
        ...
        let hash = getHash();
        if (!mustUpdateLocationHash() && forcedHash) hash = forcedHash;
        if (hash.replace(/^#/, "") !== url) {
            // push request in the queue
            queue(url, args, store);
            if (mustUpdateLocationHash()) setHash(url);
            else {
                forcedHash = url;
    ...
    const queue = function(hash) {
        ...
        if (!navigateQueue.has(hash)) {
            for (let request of navigateQueue.values())request.cancel();
            const request1 = createRequest(hash, args, store);
            navigateQueue.set(decodeURIComponent(hash), request1);
            return request1;
        }
        return false;
    };
   ...
   */ const handleHashChange = async (override)=>{
        ...
        if (!request && !navigateQueue.size) request = queue(hash);
        const route = getRouteByHash(hash);
        if (!route) {
            if (routeExists("*")) navigate("*", {
                failedHash: hash
            });
            else console.error("Unable to navigate to: ".concat(hash));
            return;
        } // update current processed request
        request.hash = hash;
        request.route = route;
        let result = await beforeEachRoute(getActiveHash(), request); // test if a local hook is configured for the route
        if (result && route.beforeNavigate) result = await route.beforeNavigate(getActiveHash(), request);
        if (isBoolean(result)) {
            // only if resolve value is explicitly true
            // we continue the current route request
            if (result) return resolveHashChange(request);
        } else {
            // if navigation guard didn't return true
            // we cancel the current request
            request.cancel();
            navigateQueue.delete(queueId);
            if (isString(result)) navigate(result);
            else if (isObject(result)) {
                let store = true;
                if (isBoolean(result.store)) store = result.store;
   ...
   */ const resolveHashChange = (request)=>{
        ...
        if (route.path) {
            ...
            if (isFunction(route.hook)) {
                ...
                const params = {};
                for (const key of urlParams.keys())params[key] = urlParams.get(key);
                route.hook(app, {
                    ...params
                });
            ...
            if (component) {
                ...
                if (activePage) {
                    ...
                    if (activePage && route.path === getActiveRoute() && !keepAlive) activePage._setState("");
                }
                if (isPage(component)) load(request).then(()=>{
                    app._refocus();
                    navigateQueue.delete(queueId);
                });
                else // of the component is not a constructor
            ...
            } else navigateQueue.delete(queueId);
        }
    };
    /**
   ...
   */ const step = function() {
        ...
        if (level > history.length) {
            if (isFunction(app._handleAppClose)) return app._handleAppClose();
            ...
            if (floor > 1) while(floor--){
                // strip of last part
                hash = hash.replace(hashLastPart, ""); // if we have a configured route
                // we navigate to it
                if (getRouteByHash(hash)) return navigate(hash, {
                    [symbols.backtrack]: true
                }, false);
            }
        }
   ...
   */ const resume = ()=>{
        if (isString(resumeHash)) {
            ...
            resumeHash = "";
        } else if (isFunction(resumeHash)) resumeHash().then((res)=>{
            resumeHash = "";
            if (isObject(res)) navigate(res.path, res.params);
            else navigate(res);
        });
        else console.warn("[Router]: resume() called but no hash found");
    };
    /**
   ...
   */ const isNavigating = ()=>{
        if (navigateQueue.size) {
            ...
            for (let request of navigateQueue.values())if (!request.isCancelled) isProcessing = true;
            return isProcessing;
        }
        return false;
    };
   ...
   */ const initRouter = (config)=>{
        if (config.getHash) getHash = config.getHash;
        if (config.setHash) setHash = config.setHash;
    };
    /**
   * On hash change we start processing
   */ const registerListener = ()=>{
        Registry.addEventListener(window, "hashchange", async ()=>{
            if (mustUpdateLocationHash()) try {
...
//# sourceMappingURL=index.502008c6.js.map
